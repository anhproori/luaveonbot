...
async def is_verified(context, user_id) -> bool:
    """Kiểm tra user đã tham gia kênh xác minh chưa. Cờ `verified` được cache
    lại trong DB để không phải gọi API Telegram mỗi lần bấm nút - cờ này chỉ
    bị reset về 0 khi: (1) admin đổi sang kênh xác minh MỚI (khác kênh cũ),
    hoặc (2) admin bấm nút "Yêu cầu xác minh lại" trong Admin Panel. Áp dụng
    y hệt cho TẤT CẢ mọi người, không có ngoại lệ cho ADMIN_ID."""
    ...
    try:
        ...
        if ok:
            ...
            # ngay lúc user được mời xác minh kênh thành công lần đầu)
...
async def admin_router(update: Update, context: ContextTypes.DEFAULT_TYPE, data: str):
    ...
    elif data.startswith("adm_chsetactive_"):
        idx = int(data.rsplit("_", 1)[1])
        channels = list_channels()
        if 0 <= idx < len(channels):
            ch = channels[idx]
            old_channel_id = cfg("channel_id", "")
            set_cfg("channel_id", ch["chat_id"])
            set_cfg("channel_title", ch["title"])
            if old_channel_id and old_channel_id != ch["chat_id"]:
                reset_all_verified()
        await show_channel_admin(update, context)

    elif data == "adm_channel_clear":
        set_cfg("channel_id", "")
        set_cfg("channel_title", "")
        await render_cb(
            update,
            "🗑✅ <b>ĐÃ GỠ BỎ KÊNH XÁC MINH</b>\n━━━━━━━━━━━━━━━\n"
            "Bot sẽ không bắt buộc tham gia kênh nào nữa cho tới khi bạn "
            "thêm bot làm admin vào 1 kênh/nhóm mới.",
            admin_menu_kb(),
        )
...
async def show_channel_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if channel_id:
        status_line = f"📡 Kênh xác minh hiện tại: <b>{channel_title or channel_id}</b>\n🆔 <code>{channel_id}</code>"
    else:
        status_line = "📡 Kênh xác minh hiện tại: <b>Chưa cấu hình</b> (bot không bắt buộc tham gia kênh nào)"
    ...
    if channels:
        ...
        for ch in channels:
            ...
            is_current = " ⭐ (đang dùng)" if ch["chat_id"] == channel_id else ""
            lines.append(f"• <b>{ch['title']}</b> — {icon}{is_current}\n   🆔 <code>{ch['chat_id']}</code>")
    ...
    text = (
        ...
        f"{channels_block}\n"
        "━━━━━━━━━━━━━━━\n"
        "👉 Để <b>đổi/đặt kênh mới</b>: thêm bot vào kênh/nhóm với quyền "
        "admin, bot sẽ tự nhận làm kênh xác minh (và tự yêu cầu mọi người "
        "xác minh lại nếu đây là kênh khác kênh cũ). Bạn cũng có thể bấm "
        "thẳng vào 1 kênh trong danh sách bên dưới để đặt làm kênh xác minh.\n\n"
        "👉 <b>🔁 Yêu cầu xác minh lại toàn bộ</b>: buộc <b>mọi người, kể cả "
        "admin</b>, phải xác nhận tham gia kênh lại ngay từ lần thao tác "
        "tiếp theo — dùng được bất cứ lúc nào, không cần đổi kênh."
    )
    rows = []
    for idx, ch in enumerate(channels):
        if ch["chat_id"] != channel_id and ch["status"] == "active":
            rows.append([InlineKeyboardButton(f"⭐ Đặt làm kênh xác minh: {ch['title']}", callback_data=f"adm_chsetactive_{idx}")])
    ...
    if channel_id:
        rows.append([InlineKeyboardButton("🗑 Gỡ bỏ kênh xác minh", callback_data="adm_channel_clear")])
    rows.append([InlineKeyboardButton("⬅️ Quay lại", callback_data="menu_admin")])
    await render_cb(update, text, InlineKeyboardMarkup(rows))
...
async def on_my_chat_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cm.new_chat_member.status in ("administrator", "member"):
        ...
        # thêm bot, để admin xem lại được kể cả khi không tự set làm kênh
        # xác minh ngay lúc đó.
        ...
        changed = bool(old_channel_id) and old_channel_id != channel_id
        if changed:
            # Đổi sang kênh MỚI khác kênh cũ -> buộc TẤT CẢ user (kể cả
            # admin) phải xác minh lại từ đầu, tránh trường hợp user cũ
            # đã có cờ verified=1 từ kênh cũ nhưng chưa hề tham gia kênh
            # mới (đây chính là nguyên nhân khiến "/start" bỏ qua bước
            # xác minh dù đã đổi kênh).
            reset_all_verified()
        try:
            extra = "\n\n🔁 Đã yêu cầu <b>toàn bộ user (kể cả bạn)</b> xác minh lại vì kênh vừa đổi." if changed else ""
            await render(
                context, ADMIN_ID, ADMIN_ID,
                f"✅📡 <b>ĐÃ ĐẶT KÊNH XÁC MINH</b>\n━━━━━━━━━━━━━━━\n"
                f"Kênh: <b>{chat.title}</b>\nID: <code>{channel_id}</code>{extra}",
                admin_menu_kb(),
            )
        except Exception:
            pass
    else:
        # Bot bị gỡ / rời khỏi kênh (left, kicked...) -> đánh dấu "tắt" trong
        # danh sách để admin biết ngay mà không cần đoán.
