...
async def show_user_detail(update: Update, context: ContextTypes.DEFAULT_TYPE, uid: int):
    user = get_user(uid)
    if not user:
        await render_cb(update, "❌ Không tìm thấy user này.", admin_menu_kb())
        return
...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if awaiting == "ban_reason":
        ...
        await render(
            context, ADMIN_ID, update.effective_chat.id,
            f"🚫 <b>CẤM USER</b> <code>{uid}</code>\n━━━━━━━━━━━━━━━\n"
            f"📄 Lý do: <b>{text}</b>\n\nChọn thời hạn cấm:",
    ...
    if awaiting == "adjust_balance":
        ...
        try:
            ...
            await update.message.reply_text("❌ Số tiền không hợp lệ, vui lòng thao tác lại từ menu Quản lý user.")
            return
