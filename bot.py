...
async def show_channel_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if channel_id:
        status_line = f"📡 Kênh hiện tại: <b>{channel_title or channel_id}</b>\n🆔 <code>{channel_id}</code>"
    else:
        status_line = "📡 Kênh hiện tại: <b>Chưa cấu hình</b> (bot không bắt buộc tham gia kênh nào)"
    ...
    text = (
        ...
        "xác minh lại nếu đây là kênh khác kênh cũ).\n\n"
        "👉 Dùng nút <b>🔁 Yêu cầu xác minh lại</b> nếu bạn muốn buộc mọi "
        "người (kể cả bạn) xác nhận tham gia kênh lại ngay bây giờ, dù "
        "kênh không đổi."
    )
    rows = [[InlineKeyboardButton("🔁 Yêu cầu xác minh lại toàn bộ", callback_data="adm_channel_reset")]]
    if channel_id:
