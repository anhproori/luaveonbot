...
def init_db():
    ...
    c.executescript(
        ...
        CREATE TABLE IF NOT EXISTS giftboxes (
            ...
            scope TEXT,                 -- 'single' hoặc 'all'
            target_user_id INTEGER,     -- chỉ dùng khi scope='single'
            reward_type TEXT,           -- 'coin' hoặc 'money'
            min_amount INTEGER,
            max_amount INTEGER,
            created_at TEXT
        );
    ...
    # Migration: lưu lại link nhiệm vụ thật (link4m) của từng token để user
    # xem lại được trong "Lịch sử nhiệm vụ" (link + trạng thái hoàn thành).
    try:
...
def clear_last_msg(user_id, expected_msg_id=None):
    """Xoá last_msg_id trong DB (đặt về NULL) khi biết chắc tin nhắn đó
    không còn edit được nữa (đã bị xoá tay, quá cũ...). Nếu expected_msg_id
    được truyền vào, chỉ xoá khi giá trị trong DB vẫn đúng bằng nó - tránh
    trường hợp 1 request khác đã render() thành công và ghi msg_id MỚI vào
    ngay trước đó, mà request này lỡ tay xoá mất msg_id mới đó đi."""
    conn = get_conn()
    if expected_msg_id is not None:
        conn.execute(
            "UPDATE users SET last_msg_id=NULL WHERE user_id=? AND last_msg_id=?",
            (user_id, expected_msg_id),
        )
    else:
        conn.execute("UPDATE users SET last_msg_id=NULL WHERE user_id=?", (user_id,))
    conn.commit()
    conn.close()
...
def user_task_statement(user_id):
    """Sao kê nhiệm vụ của 1 user - dùng để admin đối chiếu khi duyệt rút
    tiền: tổng thu nhiệm vụ (lifetime) so với tổng đã rút (nhiệm vụ) + đang
    chờ duyệt (nhiệm vụ), để biết user có THỰC SỰ hoàn thành đủ nhiệm vụ
    tương ứng số tiền đang xin rút hay không."""
    conn = get_conn()
    tasks_done = conn.execute(
        "SELECT COUNT(*) c FROM tasks WHERE user_id=? AND used=1", (user_id,)
    ).fetchone()["c"]
    total_task_income = conn.execute(
        "SELECT COALESCE(SUM(amount),0) s FROM income_log WHERE user_id=? AND type='task'",
        (user_id,),
    ).fetchone()["s"]
    withdrawn_task = conn.execute(
        "SELECT COALESCE(SUM(amount),0) s FROM withdrawals WHERE user_id=? AND source='task' "
        "AND status IN ('approved','pending')",
        (user_id,),
    ).fetchone()["s"]
    conn.close()
    conn = get_conn()
    # admin_adjust + giftbox: tiền admin cộng tay hoặc trúng hộp quà cũng
    # được cộng vào task_balance - phải tính vào đây để KHÔNG từ chối oan
    # các user hợp lệ.
    other_task_income = conn.execute(
        "SELECT COALESCE(SUM(amount),0) s FROM income_log WHERE user_id=? AND type IN ('admin_adjust','giftbox')",
        (user_id,),
    ).fetchone()["s"]
    conn.close()
    return {
        "tasks_done": tasks_done,
        "total_task_income": total_task_income,
        "withdrawn_task": withdrawn_task,
        "available_task": total_task_income - withdrawn_task,
        "eligible_task_funds": total_task_income + other_task_income - withdrawn_task,
    }
...
def weighted_task_reward(reward_min, reward_max):
    """Random tiền thưởng nhiệm vụ theo TỈ LỆ, không còn random đều (uniform)
    như trước. Trước đây random.randint(min,max) khiến mức thưởng CAO NHẤT
    (gần max, ví dụ 500đ) có xác suất ra y hệt mức thấp (ví dụ 100đ), khiến
    admin thấy 500đ ra "quá dễ". Bản này chia khoảng [min,max] thành 4 mức,
    mức thấp (gần min) ra THƯỜNG XUYÊN nhất, mức cao nhất (gần max) ra RẤT
    HIẾM - đúng cảm giác "phần thưởng lớn là hàng hiếm"."""
    reward_min, reward_max = int(reward_min), int(reward_max)
    if reward_max <= reward_min:
        return reward_min
    span = reward_max - reward_min
    tiers = [
        (reward_min, reward_min + max(0, int(span * 0.25)), 55),      # thấp nhất - phổ biến nhất
        (reward_min + int(span * 0.25) + 1, reward_min + int(span * 0.55), 27),
        (reward_min + int(span * 0.55) + 1, reward_min + int(span * 0.8), 13),
        (reward_min + int(span * 0.8) + 1, reward_max, 5),           # cao nhất (gần max) - hiếm nhất
    ]
    valid = [(lo, hi, w) for lo, hi, w in tiers if lo <= hi]
    if not valid:
        return random.randint(reward_min, reward_max)
    lo, hi, _w = random.choices(valid, weights=[t[2] for t in valid], k=1)[0]
...
async def link4m_shorten(api_key: str, dest_url: str):
    ...
    shortened = (data.get("shortenedUrl") or "").strip()
    if not shortened:
        return False, "link4m trả về thành công nhưng thiếu shortenedUrl."
    if not shortened.startswith(("http://", "https://")):
        shortened = "https://" + shortened
    ...
    async with _lock_for(user_id):
        ...
        if msg_id:
            try:
                ...
                # Tin cũ không edit được nữa (user đã xoá tay tin của bot,
                # tin quá cũ, bot từng bị chặn...). QUAN TRỌNG: xoá
                # last_msg_id trong DB NGAY LÚC NÀY, trước khi gửi tin mới.
                # Trước đây last_msg_id CHỈ được cập nhật sau khi send_message
                # thành công ở cuối hàm - nếu send_message phía dưới cũng lỗi
                # (mất mạng tạm thời, bot bị chặn, Telegram rate-limit...)
                # thì last_msg_id CŨ (đã chết) vẫn còn nguyên trong DB. Lần
                # /start kế tiếp của user lại cố edit đúng msg_id đã chết đó,
                # thất bại y hệt, rồi lại thử gửi tin mới... cứ thế lặp lại -
                # đây chính là cảm giác "bấm start liên tục không phản hồi"
                # sau khi tin nhắn bot bị xoá. Xoá last_msg_id ngay từ bước
                # này đảm bảo dù nhánh send_message bên dưới có lỡ thất bại,
                # lần gọi kế tiếp cũng sẽ gửi tin mới thẳng luôn (không tốn
                # công thử edit lại 1 tin đã biết chắc là chết).
                clear_last_msg(user_id, expected_msg_id=msg_id)
                try:
...
def main_menu_kb(user_id):
    rows = [
        ...
        [InlineKeyboardButton("📝 Lịch sử nhiệm vụ", callback_data="menu_taskhistory")],
    ]
    if user_id == ADMIN_ID:
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Lưới an toàn cuối cùng cho riêng /start: nếu BẤT KỲ bước nào ở trên
        # ném lỗi không lường trước (DB tạm lỗi, Telegram tạm lỗi...), không
        # để rơi vào tay app.add_error_handler(on_error) một cách âm thầm -
        # vì on_error chỉ gửi 1 tin chung chung "bấm /start để tiếp tục", mà
        # nếu nguyên nhân gốc vẫn còn (ví dụ last_msg_id trong DB đang trỏ
        # tới 1 tin đã chết) thì lần bấm /start kế tiếp sẽ lỗi y hệt -> đúng
        # cảm giác "vòng lặp start liên tục, không phản hồi" mà user gặp.
        # Ở đây: xoá sạch last_msg_id đang lưu (dù đúng hay sai) rồi gửi
        # THẲNG 1 tin nhắn mới, không qua edit - đảm bảo lần bấm /start này
        # chắc chắn có phản hồi và tự phá vỡ vòng lặp cho lần bấm kế tiếp.
        logger.exception("cmd_start(): lỗi không lường trước cho user_id=%s", user_tg.id)
        clear_last_msg(user_tg.id)
        try:
            user = get_user(user_tg.id)
            sent = await context.bot.send_message(
                chat_id=chat_id,
                text=welcome_text(user) if user else "👋 Chào bạn! Bấm /start lại 1 lần nữa nhé.",
                reply_markup=main_menu_kb(user_tg.id) if user else None,
                parse_mode="HTML", disable_web_page_preview=True,
            )
            set_last_msg(user_tg.id, sent.message_id)
        except Exception:
            logger.exception("cmd_start(): lưới an toàn cũng thất bại cho user_id=%s", user_tg.id)
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data == "menu_taskhistory":
        await show_task_history(update, context)

    elif data.startswith("gb_"):
        await claim_giftbox_cb(update, context, int(data.split("_", 1)[1]))
...
async def show_task_history(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = get_conn()
    rows = conn.execute(
        "SELECT rowid AS rid, * FROM tasks WHERE user_id=? ORDER BY rid DESC LIMIT 10",
        (user_id,),
    ).fetchall()
    conn.close()

    if not rows:
        text = "📝 <b>LỊCH SỬ NHIỆM VỤ</b>\n━━━━━━━━━━━━━━━\nBạn chưa làm nhiệm vụ nào."
    else:
        lines = []
        for i, r in enumerate(rows, 1):
            status = "✅ Đã hoàn thành" if r["used"] else "⏳ Chưa hoàn thành"
            link = r["task_link"] or ""
            link_line = f'<a href="{link}">🔗 Link nhiệm vụ #{i}</a>' if link else f"🔗 Link nhiệm vụ #{i} (không rõ)"
            reward_line = f"+{money_fmt(r['reward_money'])} / +{r['reward_coin']} coin" if r["used"] else "(ẩn tới khi hoàn thành)"
            lines.append(
                f"{link_line} — {status}\n   🎁 {reward_line}\n   <i>{fmt_vn(r['created_at'])}</i>"
            )
        text = (
            "📝 <b>LỊCH SỬ NHIỆM VỤ</b> (10 gần nhất)\n"
            "━━━━━━━━━━━━━━━\n" + "\n\n".join(lines)
        )
    await render_cb(update, text, back_kb())
...
async def do_task(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reward_money = weighted_task_reward(cfg("reward_min", 150), cfg("reward_max", 500))
    ...
    conn = get_conn()
    conn.execute("UPDATE tasks SET task_link=? WHERE token=?", (task_link, token))
    conn.commit()
    conn.close()
    ...
    text = (
        "📝✨ <b>NHIỆM VỤ MỚI</b> ✨\n"
        "━━━━━━━━━━━━━━━\n"
        "🎁 Phần thưởng: <b>🎲 BÍ ẨN - hoàn thành nhiệm vụ để biết bạn nhận bao nhiêu!</b>\n\n"
...
async def create_withdrawal(update: Update, context: ContextTypes.DEFAULT_TYPE, amount: int):
    ...
    # Đối chiếu "sao kê nhiệm vụ": nếu tiền nhiệm vụ user thực sự kiếm được
    # (từ nhiệm vụ đã hoàn thành, cộng cả phần admin cộng tay nếu có) KHÔNG
    # đủ để phủ số tiền đang xin rút -> tự động từ chối ngay, không cần admin
    # can thiệp (an toàn: chỉ chặn khi có bất thường thật sự trong dữ liệu).
    if source == "task":
        stmt = user_task_statement(user_id)
        if stmt["eligible_task_funds"] < amount:
            conn = get_conn()
            conn.execute(
                "INSERT INTO withdrawals (user_id, amount, source, bank_info, status, reject_reason, created_at) "
                "VALUES (?,?,?,?,?,?,?)",
                (user_id, amount, source, user["bank_info"], "rejected",
                 "Tự động từ chối: thống kê nhiệm vụ hoàn thành chưa đủ giá trị số tiền yêu cầu rút.",
                 now_vn().isoformat()),
            )
            conn.commit()
            conn.close()
            await render_cb(
                update,
                "❌🤖 <b>YÊU CẦU RÚT TIỀN BỊ TỪ CHỐI TỰ ĐỘNG</b>\n"
                "━━━━━━━━━━━━━━━\n"
                "Thống kê nhiệm vụ hoàn thành của bạn chưa đủ giá trị so với số "
                "tiền đang xin rút. Vui lòng làm thêm nhiệm vụ rồi thử lại.",
                back_kb(),
            )
            return
    ...
    await render(
        ...
        "━━━━━━━━━━━━━━━\n"
        f"📊 <b>Sao kê nhiệm vụ:</b> đã làm {stmt['tasks_done']} nhiệm vụ, "
        f"tổng thu {money_fmt(stmt['total_task_income'])}, đã rút (nhiệm vụ) "
        f"{money_fmt(stmt['withdrawn_task'])}",
        kb,
    )
...
def claim_giftbox(giftbox_id: int, user_id: int):
    """Xử lý 1 lượt nhận hộp quà. Trả về (amount, reward_type) nếu thành
    công, hoặc (None, error_code) nếu thất bại. error_code thuộc:
    'notfound' | 'notyours' | 'claimed'."""
    conn = get_conn()
    gb = conn.execute("SELECT * FROM giftboxes WHERE id=?", (giftbox_id,)).fetchone()
    if not gb:
        conn.close()
        return None, "notfound"
    if gb["scope"] == "single" and gb["target_user_id"] != user_id:
        conn.close()
        return None, "notyours"
    existing = conn.execute(
        "SELECT id FROM giftbox_claims WHERE giftbox_id=? AND user_id=?", (giftbox_id, user_id)
    ).fetchone()
    if existing:
        conn.close()
        return None, "claimed"
    amount = random.randint(int(gb["min_amount"]), int(gb["max_amount"]))
    reward_type = gb["reward_type"]
    conn.execute(
        "INSERT INTO giftbox_claims (giftbox_id, user_id, amount, claimed_at) VALUES (?,?,?,?)",
        (giftbox_id, user_id, amount, now_vn().isoformat()),
    )
    conn.commit()
    conn.close()

    if reward_type == "money":
        add_balance(user_id, amount, 0, "task", "giftbox")
    else:
        conn = get_conn()
        conn.execute("UPDATE users SET coin = coin + ? WHERE user_id=?", (amount, user_id))
        conn.commit()
        conn.close()
    return amount, reward_type


async def claim_giftbox_cb(update: Update, context: ContextTypes.DEFAULT_TYPE, giftbox_id: int):
    user_id = update.effective_user.id
    amount, result = claim_giftbox(giftbox_id, user_id)
    if amount is None:
        msgs = {
            "notfound": "❌ Hộp quà không tồn tại hoặc đã bị gỡ.",
            "notyours": "🚫 Hộp quà này không dành cho bạn.",
            "claimed": "⚠️ Bạn đã nhận hộp quà này rồi, không thể nhận lại lần nữa.",
        }
        await q_answer(update, msgs.get(result, "❌ Có lỗi xảy ra."))
        return
    reward_text = f"{amount} coin" if result == "coin" else money_fmt(amount)
    user = get_user(user_id)
    await render_cb_fx(
        update,
        "🎁 <i>Đang mở hộp quà...</i> ✨🎉",
        "🎉🎁 <b>CHÚC MỪNG! BẠN VỪA MỞ HỘP QUÀ</b> 🎁🎉\n"
        "━━━━━━━━━━━━━━━\n"
        f"🎁 Nhận được: <b>+{reward_text}</b>\n\n"
        f"💰 Số dư hiện tại: <b>{money_fmt(user['task_balance'] + user['ref_balance'])}</b>\n"
        f"🪙 Coin hiện tại: <b>{user['coin']}</b>",
        main_menu_kb(user_id),
    )
...
GB_TYPE_LABEL = {"coin": "🪙 Coin", "money": "💰 Tiền"}
GB_SCOPE_LABEL = {"single": "👤 1 user", "all": "🌍 Tất cả user"}


async def show_giftbox_history(update: Update, context: ContextTypes.DEFAULT_TYPE, page: int = 0):
    per_page = 8
    conn = get_conn()
    total = conn.execute("SELECT COUNT(*) c FROM giftboxes").fetchone()["c"]
    rows = conn.execute(
        "SELECT * FROM giftboxes ORDER BY id DESC LIMIT ? OFFSET ?",
        (per_page, page * per_page),
    ).fetchall()
    conn.close()

    if not rows:
        await render_cb(update, "📜 <b>LỊCH SỬ HỘP QUÀ</b>\n━━━━━━━━━━━━━━━\nChưa có hộp quà nào được tạo.", admin_menu_kb())
        return
    ...
    for gb in rows:
        claims = conn.execute(
            "SELECT COUNT(*) c, COALESCE(SUM(amount),0) s FROM giftbox_claims WHERE giftbox_id=?", (gb["id"],)
        ).fetchone()
        scope_line = GB_SCOPE_LABEL.get(gb["scope"], gb["scope"])
        if gb["scope"] == "single":
            scope_line += f" (<code>{gb['target_user_id']}</code>)"
        lines.append(
            f"🎁 #{gb['id']} — {scope_line} — {GB_TYPE_LABEL.get(gb['reward_type'], gb['reward_type'])} "
            f"({gb['min_amount']}-{gb['max_amount']})\n"
            f"   ✅ Đã nhận: {claims['c']} lượt — tổng {claims['s']}\n"
            f"   <i>{fmt_vn(gb['created_at'])}</i>"
        )
    conn.close()
    text = (
        f"📜✨ <b>LỊCH SỬ HỘP QUÀ</b> ✨\n━━━━━━━━━━━━━━━\n"
        f"Tổng: <b>{total}</b> hộp quà | Trang <b>{page + 1}/{max_page + 1}</b>\n\n"
        + "\n\n".join(lines)
    )
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("⬅️ Trước", callback_data=f"adm_gbhistory_{page - 1}"))
    if page < max_page:
        nav.append(InlineKeyboardButton("Sau ➡️", callback_data=f"adm_gbhistory_{page + 1}"))
...
def admin_menu_kb():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🎉 Tặng Hộp Quà", callback_data="adm_giftbox"),
         InlineKeyboardButton("📜 Lịch sử Hộp Quà", callback_data="adm_gbhistory_0")],
...
async def admin_router(update: Update, context: ContextTypes.DEFAULT_TYPE, data: str):
    if data == "adm_broadcast":
    ...
    elif data == "adm_giftbox":
        context.user_data.pop("awaiting", None)
        context.user_data.pop("gb_scope", None)
        context.user_data.pop("gb_target", None)
        context.user_data.pop("gb_type", None)
        context.user_data.pop("gb_min", None)
        await render_cb(
            update,
            "🎉🎁 <b>TẶNG HỘP QUÀ</b>\n━━━━━━━━━━━━━━━\n"
            "Hộp quà sẽ trả thưởng <b>hoàn toàn ngẫu nhiên</b> trong khoảng "
            "min/max bạn đặt, mỗi user chỉ nhận được đúng 1 lần.\n\n"
            "Chọn đối tượng nhận quà:",
            InlineKeyboardMarkup([
                [InlineKeyboardButton("👤 1 user cụ thể", callback_data="adm_gbscope_single")],
                [InlineKeyboardButton("🌍 Tất cả user", callback_data="adm_gbscope_all")],
                [InlineKeyboardButton("⬅️ Huỷ", callback_data="menu_admin")],
            ]),
        )

    elif data == "adm_gbscope_single":
        context.user_data["gb_scope"] = "single"
        context.user_data["awaiting"] = "giftbox_target_id"
        await render_cb(update, "👤 Nhập <b>ID Telegram</b> của user sẽ nhận hộp quà:", back_kb())

    elif data == "adm_gbscope_all":
        context.user_data["gb_scope"] = "all"
        context.user_data["gb_target"] = None
        await render_cb(
            update,
            "🌍 Hộp quà sẽ gửi cho <b>TẤT CẢ user</b>.\n\nChọn loại phần thưởng:",
            InlineKeyboardMarkup([
                [InlineKeyboardButton("🪙 Coin", callback_data="adm_gbtype_coin"),
                 InlineKeyboardButton("💰 Tiền", callback_data="adm_gbtype_money")],
                [InlineKeyboardButton("⬅️ Huỷ", callback_data="menu_admin")],
            ]),
        )

    elif data in ("adm_gbtype_coin", "adm_gbtype_money"):
        reward_type = "coin" if data.endswith("coin") else "money"
        context.user_data["gb_type"] = reward_type
        context.user_data["awaiting"] = "giftbox_min"
        unit = "coin" if reward_type == "coin" else "đồng"
        await render_cb(update, f"✏️ Nhập mức <b>tối thiểu</b> ({unit}) user có thể nhận được:", back_kb())

    elif data.startswith("adm_gbhistory_"):
        page = int(data.rsplit("_", 1)[1])
        await show_giftbox_history(update, context, page)

    elif data.startswith("adm_editname_"):
        tool_id = int(data.split("_", 2)[2])
        context.user_data["awaiting"] = "edit_tool_name"
        context.user_data["edit_tool_id"] = tool_id
        await render_cb(update, "✏️ Nhập <b>tên mới</b> cho Tài Nguyên này:", back_kb())

    elif data.startswith("adm_editaccounts_"):
        tool_id = int(data.split("_", 2)[2])
        context.user_data["awaiting"] = "edit_tool_accounts"
        context.user_data["edit_tool_id"] = tool_id
        await render_cb(
            update,
            "✏️ Gửi <b>toàn bộ danh sách tài khoản mới</b> (sẽ THAY THẾ hoàn toàn "
            "danh sách cũ), mỗi dòng 1 tài khoản dạng <code>user:pass</code>:",
            back_kb(),
        )
        ...
        min_task = cfg("min_withdraw_task", 10000)
        min_ref = cfg("min_withdraw_ref", 50000)
        text = (
            "⚙️ <b>CẤU HÌNH</b>\n"
            "━━━━━━━━━━━━━━━\n"
            f"💸 Min rút hiện tại: nhiệm vụ {money_fmt(min_task)} | ref {money_fmt(min_ref)}\n"
            f"🎁 Mức thưởng nhiệm vụ: {money_fmt(cfg('reward_min'))} - {money_fmt(cfg('reward_max'))}\n\n"
            "👉 Bấm nút bên dưới để sửa nhanh, hoặc gõ lệnh trực tiếp:\n"
        ...
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("✏️ Sửa Min rút tiền", callback_data="adm_editminwd")],
            [InlineKeyboardButton("✏️ Sửa mức thưởng nhiệm vụ", callback_data="adm_editreward")],
            [InlineKeyboardButton("⬅️ Quay lại", callback_data="menu_admin")],
        ])
        await render_cb(update, text, kb)

    elif data == "adm_editminwd":
        context.user_data["awaiting"] = "set_minwd_task"
        await render_cb(update, "✏️ Nhập <b>min rút (tiền nhiệm vụ)</b>, ví dụ <code>10000</code>:", back_kb())

    elif data == "adm_editreward":
        context.user_data["awaiting"] = "set_reward_min"
        await render_cb(update, "✏️ Nhập mức thưởng <b>tối thiểu</b> nhiệm vụ (đồng), ví dụ <code>50</code>:", back_kb())
...
async def show_admin_tools(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for t in tools:
        ...
        rows.append([
            InlineKeyboardButton("✏️ Đổi tên", callback_data=f"adm_editname_{t['id']}"),
            InlineKeyboardButton("✏️ Sửa DS tài khoản", callback_data=f"adm_editaccounts_{t['id']}"),
        ])
...
def user_detail_view(uid: int):
    ...
    tstmt = user_task_statement(uid)
    total_withdrawn = 0
    conn = get_conn()
    row = conn.execute(
        "SELECT COALESCE(SUM(amount),0) s FROM withdrawals WHERE user_id=? AND status='approved'", (uid,)
    ).fetchone()
    total_withdrawn = row["s"]
    conn.close()
    ...
    text = (
        ...
        "━━━━━━━━━━━━━━━\n"
        f"📊 <b>Sao kê nhiệm vụ:</b> tổng thu {money_fmt(tstmt['total_task_income'])}, "
        f"đã rút (nhiệm vụ) {money_fmt(tstmt['withdrawn_task'])}\n"
        f"💵 <b>Tổng đã rút (đã duyệt, mọi loại):</b> {money_fmt(total_withdrawn)}\n"
...
def _pending_withdrawals_view():
    ...
    for r in shown:
        stmt_line = ""
        if r["source"] == "task":
            stmt = user_task_statement(r["user_id"])
            stmt_line = (
                f"📊 Sao kê: {stmt['tasks_done']} nhiệm vụ, tổng thu "
                f"{money_fmt(stmt['total_task_income'])}, đã rút {money_fmt(stmt['withdrawn_task'])}\n"
            )
        text += (
            f"🧾 #{r['id']} — user <code>{r['user_id']}</code> — <b>{money_fmt(r['amount'])}</b>\n"
            f"🏦 {r['bank_info']}\n{stmt_line}<i>{fmt_vn(r['created_at'])}</i>\n\n"
        )
        kb_rows.append([
...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if awaiting == "edit_tool_name":
        tool_id = context.user_data.pop("edit_tool_id", None)
        context.user_data.pop("awaiting", None)
        await try_delete_message(update)
        conn = get_conn()
        conn.execute("UPDATE tools SET name=? WHERE id=?", (text, tool_id))
        conn.commit()
        conn.close()
        await render(context, ADMIN_ID, chat_id, f"✅ Đã đổi tên Tài Nguyên thành <b>{text}</b>.", admin_menu_kb())
        return

    if awaiting == "edit_tool_accounts":
        tool_id = context.user_data.pop("edit_tool_id", None)
        context.user_data.pop("awaiting", None)
        await try_delete_message(update)
        conn = get_conn()
        conn.execute("UPDATE tools SET accounts=? WHERE id=?", (text, tool_id))
        conn.commit()
        conn.close()
        count = len([a for a in text.split("\n") if a.strip()])
        await render(
            context, ADMIN_ID, chat_id,
            f"✅✏️ <b>ĐÃ THAY TOÀN BỘ DANH SÁCH TÀI KHOẢN</b>\nCòn lại: {count} tài khoản.",
            admin_menu_kb(),
        )
        return

    if awaiting == "set_minwd_task":
        await try_delete_message(update)
        if not text.isdigit():
            await render(context, ADMIN_ID, chat_id, "❌ Vui lòng nhập số.", back_kb())
            return
        context.user_data["minwd_task_val"] = text
        context.user_data["awaiting"] = "set_minwd_ref"
        await render(context, ADMIN_ID, chat_id, "✏️ Nhập <b>min rút (tiền ref)</b>, ví dụ <code>50000</code>:", back_kb())
        return

    if awaiting == "set_minwd_ref":
        await try_delete_message(update)
        if not text.isdigit():
            await render(context, ADMIN_ID, chat_id, "❌ Vui lòng nhập số.", back_kb())
            return
        min_task = context.user_data.pop("minwd_task_val", "10000")
        context.user_data.pop("awaiting", None)
        set_cfg("min_withdraw_task", min_task)
        set_cfg("min_withdraw_ref", text)
        await render(
            context, ADMIN_ID, chat_id,
            f"✅ Đã cập nhật min rút tiền: nhiệm vụ {money_fmt(int(min_task))} | ref {money_fmt(int(text))}.",
            admin_menu_kb(),
        )
        return

    if awaiting == "set_reward_min":
        await try_delete_message(update)
        if not text.isdigit():
            await render(context, ADMIN_ID, chat_id, "❌ Vui lòng nhập số.", back_kb())
            return
        context.user_data["reward_min_val"] = text
        context.user_data["awaiting"] = "set_reward_max"
        await render(context, ADMIN_ID, chat_id, "✏️ Nhập mức thưởng <b>tối đa</b> nhiệm vụ (đồng), ví dụ <code>500</code>:", back_kb())
        return

    if awaiting == "set_reward_max":
        await try_delete_message(update)
        if not text.isdigit():
            await render(context, ADMIN_ID, chat_id, "❌ Vui lòng nhập số.", back_kb())
            return
        reward_min = context.user_data.pop("reward_min_val", "50")
        context.user_data.pop("awaiting", None)
        set_cfg("reward_min", reward_min)
        set_cfg("reward_max", text)
        await render(
            context, ADMIN_ID, chat_id,
            f"✅ Đã cập nhật mức thưởng nhiệm vụ: {money_fmt(int(reward_min))} - {money_fmt(int(text))}.",
            admin_menu_kb(),
        )
        return

    if awaiting == "giftbox_target_id":
        context.user_data.pop("awaiting", None)
        await try_delete_message(update)
        if not text.isdigit():
            await render(context, ADMIN_ID, chat_id, "❌ ID không hợp lệ, vui lòng chỉ nhập số.", admin_menu_kb())
        ...
        if not get_user(target_uid):
            ...
            return
        context.user_data["gb_target"] = target_uid
        await render(
            context, ADMIN_ID, chat_id,
            f"👤 User nhận quà: <code>{target_uid}</code>\n\nChọn loại phần thưởng:",
            InlineKeyboardMarkup([
                [InlineKeyboardButton("🪙 Coin", callback_data="adm_gbtype_coin"),
                 InlineKeyboardButton("💰 Tiền", callback_data="adm_gbtype_money")],
                [InlineKeyboardButton("⬅️ Huỷ", callback_data="menu_admin")],
    ...
    if awaiting == "giftbox_min":
        await try_delete_message(update)
        if not text.isdigit():
            await render(context, ADMIN_ID, chat_id, "❌ Vui lòng nhập số.", back_kb())
            return
        context.user_data["gb_min"] = int(text)
        context.user_data["awaiting"] = "giftbox_max"
        await render(context, ADMIN_ID, chat_id, "✏️ Nhập mức <b>tối đa</b> user có thể nhận được:", back_kb())
        return

    if awaiting == "giftbox_max":
        await try_delete_message(update)
        if not text.isdigit():
            await render(context, ADMIN_ID, chat_id, "❌ Vui lòng nhập số.", back_kb())
            return
        gmax = int(text)
        gmin = context.user_data.pop("gb_min", 0)
        scope = context.user_data.pop("gb_scope", "all")
        target = context.user_data.pop("gb_target", None)
        reward_type = context.user_data.pop("gb_type", "coin")
        context.user_data.pop("awaiting", None)
        if gmax < gmin:
            gmin, gmax = gmax, gmin
        ...
        conn = get_conn()
        conn.execute(
            "INSERT INTO giftboxes (scope, target_user_id, reward_type, min_amount, max_amount, created_at) "
            "VALUES (?,?,?,?,?,?)",
            (scope, target, reward_type, gmin, gmax, now_vn().isoformat()),
        )
        gb_id = conn.execute("SELECT last_insert_rowid() id").fetchone()["id"]
        if scope == "all":
            recipients = [r["user_id"] for r in conn.execute("SELECT user_id FROM users").fetchall()]
        else:
        ...
        unit = "coin" if reward_type == "coin" else "đồng"
        gift_kb = InlineKeyboardMarkup([[InlineKeyboardButton("🎁 Nhận hộp quà ngay!", callback_data=f"gb_{gb_id}")]])
        gift_text = (
            "🎉🎁 <b>BẠN VỪA NHẬN ĐƯỢC 1 HỘP QUÀ!</b> 🎁🎉\n"
            "━━━━━━━━━━━━━━━\n"
            f"🎲 Phần thưởng ngẫu nhiên: <b>{gmin}-{gmax} {unit}</b>\n\n"
            "👉 Bấm nút bên dưới để mở hộp quà và nhận thưởng ngay!"
        )
        sent, failed = 0, 0
        for uid in recipients:
            try:
                await render(context, uid, uid, gift_text, gift_kb)
                sent += 1
            except Exception:
                failed += 1
        ...
        await render(
            context, ADMIN_ID, chat_id,
            f"✅🎉 <b>ĐÃ TẠO HỘP QUÀ #{gb_id}</b>\n━━━━━━━━━━━━━━━\n"
            f"🎯 Đối tượng: {GB_SCOPE_LABEL.get(scope, scope)}\n"
            f"🎁 Loại: {GB_TYPE_LABEL.get(reward_type, reward_type)}\n"
            f"🎲 Khoảng: {gmin}-{gmax}\n"
            f"📤 Đã gửi thông báo: {sent} user (lỗi: {failed})",
            admin_menu_kb(),
        )
        return
