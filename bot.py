...
async def do_checkin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    bonus = random.randint(int(cfg("checkin_min", 100)), int(cfg("checkin_max", 300)))
    conn = get_conn()
    conn.execute("UPDATE users SET last_checkin=? WHERE user_id=?", (today, user_id))
    conn.commit()
    conn.close()
...
async def show_leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, r in enumerate(rows):
        ...
        lines.append(f"{icon} <b>{name}</b> — {money_fmt(r['total'])}")
