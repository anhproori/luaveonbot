...
## 🆕 Bản cập nhật MỚI NHẤT (fix tỉ lệ thưởng + bí ẩn phần thưởng + Hộp quà + sao kê rút tiền)
- **🎲 Fix tỉ lệ thưởng nhiệm vụ**: trước đây `random.randint(min,max)` khiến
  mức thưởng CAO NHẤT (gần max, vd 500đ) có xác suất ra y hệt mức thấp (vd
  100đ). Bản này chia khoảng thưởng thành 4 mức theo trọng số (~55% / 27% /
  13% / 5%): mức thấp (gần min) ra THƯỜNG XUYÊN, mức cao nhất (gần max) ra
  RẤT HIẾM — đúng cảm giác "phần thưởng lớn là hàng hiếm". Chỉnh nhanh bằng
  nút **"✏️ Sửa mức thưởng nhiệm vụ"** trong Admin Panel → Cấu hình.
- **🎁 Ẩn phần thưởng trước khi làm nhiệm vụ**: màn "Nhiệm vụ mới" giờ chỉ
  hiện "🎲 Bí ẩn — hoàn thành để biết", số tiền/coin thật chỉ lộ ra sau khi
  hoàn thành nhiệm vụ (tăng cảm giác hồi hộp, tránh user chỉ chọn làm khi
  thấy mức thưởng cao).
- **✏️ Admin chỉnh Min rút tiền bằng nút bấm**: thêm nút "✏️ Sửa Min rút
  tiền" trong Admin Panel → Cấu hình, không cần gõ lệnh `/setminwd` nữa.
- **🎁 Quản lý Tài Nguyên đầy đủ CRUD**: thêm nút "✏️ Đổi tên" và "✏️ Sửa DS
  tài khoản" (thay toàn bộ danh sách 1 lần) bên cạnh add/sửa giá/xoá/nạp
  thêm đã có sẵn.
- **🎉 Tính năng MỚI: Tặng Hộp Quà** — admin có thể tặng quà ngẫu nhiên cho
  1 user cụ thể hoặc TẤT CẢ user: chọn loại thưởng (Coin | Tiền), nhập
  min/max, mỗi user bấm "🎁 Nhận hộp quà" sẽ được cộng 1 số ngẫu nhiên trong
  khoảng đó, chỉ nhận được đúng 1 lần/hộp quà. Admin xem lại lịch sử phát
  quà + số lượt nhận qua màn "📜 Lịch sử Hộp Quà".
- **📝 Lịch sử hoàn thành nhiệm vụ**: nút mới "📝 Lịch sử nhiệm vụ" ở menu
  chính, hiện link nhiệm vụ đã tạo kèm trạng thái Đã hoàn thành / Chưa hoàn
  thành.
- **💸 Cập nhật duyệt rút tiền — đối chiếu sao kê nhiệm vụ**: khi user gửi
  yêu cầu rút tiền (nguồn "nhiệm vụ"), bot tự so tổng tiền nhiệm vụ THẬT đã
  kiếm được với số tiền đang xin rút — nếu chưa đủ sẽ **tự động từ chối
  ngay**, không cần admin can thiệp (có tính cả tiền admin cộng tay/hộp quà
  để không từ chối oan). Màn hình admin duyệt rút tiền và tin nhắn thông
  báo yêu cầu mới giờ hiện thêm dòng "📊 Sao kê nhiệm vụ" (số nhiệm vụ đã
  làm, tổng thu, đã rút) để admin đối chiếu trực quan.
- **👥 Quản lý user chi tiết hơn**: màn chi tiết user thêm dòng sao kê
  nhiệm vụ + tổng tiền đã rút (đã duyệt, mọi loại).

## 🆕 Bản cập nhật trước đó (API link4m gọi từ server + fix duyệt rút tiền + kênh xác minh nhiều kênh)
