...
## 🆕 Bản cập nhật này đã sửa/thêm gì
- **Sửa lỗi "làm nhiệm vụ" báo 403 "Just a moment..."**: đây là trang chặn
  của Cloudflare (link4m đứng sau Cloudflare). Bot giờ dùng thư viện
  `cloudscraper` (đã thêm vào `requirements.txt`) để tự vượt qua thử thách
  này, kèm giả lập header trình duyệt thật + tự thử lại tối đa 3 lần nếu vẫn
  bị chặn/lỗi mạng. Nếu vẫn lỗi, log server sẽ ghi rõ nguyên nhân.
